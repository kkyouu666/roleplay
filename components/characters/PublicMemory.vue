      ...
      <div v-for="memory in memories" :key="memory.id"
        ...
        <div class="bg-gray-50 dark:bg-gray-700 p-3 flex items-center justify-between">
          <div class="flex items-center">
            <img :src="memory.user.avatar" :alt="memory.user.username" class="w-6 h-6 rounded-full object-cover mr-2" />
        ...
        <div class="p-3 space-y-3">
          <div v-for="message in memory.messages" :key="message.id" class="flex"
            :class="message.sender === 'user' ? 'justify-end' : 'justify-start'">
            <div class="max-w-[80%] rounded-lg px-3 py-2 text-sm" :class="message.sender === 'user'
