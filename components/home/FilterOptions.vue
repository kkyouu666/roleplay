      ...
      <div v-show="showMobileFilters" class="space-y-4 transition-all duration-300 mt-4">
        ...
        <div class="relative">
          <div class="overflow-hidden">
            <div class="overflow-x-auto hide-scrollbar -mr-4 pr-4">
              <div class="flex flex-nowrap gap-2 min-w-max">
                <button v-for="tag in popularTags" :key="tag.value" @click="$emit('toggle-tag', tag.value)"
      ...
      <div class="relative mb-4">
        <div class="overflow-hidden">
          <div class="overflow-x-auto hide-scrollbar -mr-4 pr-4">
            <div class="flex flex-nowrap gap-2 min-w-max">
              <button v-for="category in categories" :key="category.value"
                @click="$emit('set-category', category.value)" class="px-3 py-1 rounded-full text-sm whitespace-nowrap"
                :class="selectedCategory === category.value
                  ? 'bg-indigo-600 text-white dark:bg-indigo-500'
                  : 'bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-600'">
                {{ $t(`roleplay.categories.${category.key}`) }}
